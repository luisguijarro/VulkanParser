...
namespace VulkanParser
{
    ...
    /// </summary>
    public static class CreaterMethodsCS2
    {
        ...
        private static void CreatePublic(StreamWriter sw, Metodo mtd)
        {
            ...
            int paramcont = 0;
            if (mtd.Nombre == "vkCreateInstance") 
            { 
                Console.WriteLine("mierda");
            }
            ...
            if (smetodo.Substring(smetodo.Length-2) == ", ")
            {
            ...
            foreach (string keyparam in mtd.Parametros.Keys) //recorrer parametros.
            {
                ...
                if (param.puntero) 
                {
                    ...
                    {
                        if (StructParser.estructuras.ContainsKey(param.TipoValor) && !param.esarray)
                        {
                    ...
                    else
                    {
                        ...
                        if (param.esarray)
                        {
                            sw.WriteLine("\t" + "\t" + "\t" + fixedtabs + "fixed(" + /*param.TipoValor*/ "char* p_" + param.Nombre + " = &" + param.Nombre + ".ToCharArray()[0])");
                        }
                        //System.Runtime.InteropServices.Marshal.StringToHGlobalAuto(param.Nombre)
                        else
                        {
                            sw.WriteLine("\t" + "\t" + "\t" + fixedtabs + "fixed(" + /*param.TipoValor*/ "char* p_" + param.Nombre + " = &" + param.Nombre + ")");
                        }
                        sw.WriteLine("\t" + "\t" + "\t" + fixedtabs + "{"); //abrir fixed;
                        l_params.Add("p_" + param.Nombre);
                        ntabs++; //añadir numero de tabulaciones
